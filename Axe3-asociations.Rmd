...
```{r clusters}
Alist <-  list(GeneExpression = Expr %>% filter(Stimulus.Name == "Null") %>% select(3:562),
           FACS = facs %>% select(!1),
           Cyto = cytokines %>% filter(StimulusName == "Null") %>% select(4:16)
         )
...
for(i in categorical_variables){ 
  # si la variable est de type caractère
    print(i)
    Tab = table(cl_h, 
                ecrf[as.character(Expr[Expr$Stimulus.Name == "BCG",]$SUBJID), i])  
    # print(Tab)
...
for(i in continuous_variables){
  ...
  anova_clin =  aov(ecrf[as.character(Expr[Expr$Stimulus.Name == "BCG",]$SUBJID), i] 
                    ~ as.factor(cl_h))
  # visualiser le résultat
  print(summary(anova_clin))
  print(TukeyHSD(anova_clin))
  # tracer un boxplot pour visualiser les résultats
  df_plot <- data.frame(cbind(var = ecrf[as.character(Expr[Expr$Stimulus.Name == "BCG",]$SUBJID), i],
